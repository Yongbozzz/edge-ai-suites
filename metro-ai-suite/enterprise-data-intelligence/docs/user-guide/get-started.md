...
```bash
# clone OPEA EC-RAG repo at a known release so the edits below apply as written
git clone --branch v1.5 --depth 1 https://github.com/opea-project/GenAIExamples.git
cd GenAIExamples/EdgeCraftRAG

compose=docker_compose/intel/gpu/arc/compose.yaml

# fail loudly if upstream no longer ships the image tag this guide rewrites,
# instead of letting sed silently do nothing (sed exits 0 when it matches nothing)
grep -q 'intel/llm-scaler-vllm:1.1-preview' "$compose" || {
  echo "ERROR: expected image tag not found in $compose; the v1.5 layout changed, update this guide" >&2
  exit 1
}
...
sed -i \
  ...
  -e 's@intel/llm-scaler-vllm:1.1-preview@intel/llm-scaler-vllm:0.21.0-b1@g' \
